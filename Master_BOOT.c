...
static uint8 XDATA Master_slReadBuf[Master_BTLDR_SIZEOF_READ_BUFFER];

/* I2C read buffer - The host reads responses from it */
static uint8 XDATA Master_slWriteBuf[Master_BTLDR_SIZEOF_WRITE_BUFFER];
...
*
* Global variables:
*  Master_slWriteBuf - The global variable used to store a received
*                                command.
*  Master_slReadBuf -  The global variable used to store a response.
...
void Master_CyBtldrCommStart(void) CYSMALL 
{
    ...
    Master_SlaveInitWriteBuf(Master_slWriteBuf, Master_BTLDR_SIZEOF_WRITE_BUFFER);
    Master_SlaveInitReadBuf (Master_slReadBuf, 0u);
...
*
*******************************************************************************/
cystatus Master_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
...
*
* Global variables:
*  Master_slWriteBuf - The global variable used to store a
*                                received command.
*
*******************************************************************************/
         ...
         
{
    ...
    if((NULL != pData) && (size > 0u))
    {
        ...
        while(0u != timeoutMs)  /* Wait for command from host */
        {
            ...
            if(0u != (Master_slStatus & Master_SSTAT_WR_CMPLT))
            {
                /* Define how many bytes host has been written */
                byteCount = Master_slWrBufIndex;
                *count = (uint16) byteCount;

                /* Copy command to pData buffer */
                (void) memcpy((void *) pData, (void *) Master_slWriteBuf,
                              Master_MIN_UNT16(byteCount, size));
